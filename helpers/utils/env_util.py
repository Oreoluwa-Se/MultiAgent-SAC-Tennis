...
class EnvironmentInit:
	...
	def make_env_func(self, seed, worker_id=0):
		"""
			initializes the environment and brain name
		"""
		env = UnityEnvironment(file_name=self.path, seed=seed, worker_id=worker_id)
		brain_name = env.brain_names[0]

		return env, brain_name

	def reset(self, env, brain_name, train_mode):
		"""
		...
		returned_params = {"next_states":env_info.vector_observations,
						   	"state_size":state_size, "num_agents":num_agents,
	...
	def step(self, env, brain_name, actions):
		...
		"""
		returned_params = {"next_states":env_info.vector_observations,
						   	"rewards":env_info.rewards, "agents":env_info.agents,
